...
import { apiService, RequirementExtraction } from '../services/api';
...
export const RequirementsInput: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        ...
        console.log(files , "files from api");
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await apiService.extractFeatures(selectedFileId);
  ...
  // Load existing requirements when file is selected
  const loadExistingRequirements = async (fileId: string) => {
    try {
      const requirements = await apiService.getRequirements(fileId);
  ...
  // Load requirements when file selection changes
  useEffect(() => {
    if (selectedFileId) {
      loadExistingRequirements(selectedFileId);
    } else {
      setExtractedRequirements([]);
      setAnalysisStatus('idle');
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="p-6">
          {activeTab === 'upload' && (
            <div className="space-y-4">
              ...
              {/* Document Selection */}
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-2">Select Uploaded Document</label>
                <select 
                ...
                {uploadedFiles.length === 0 && (
                  <p className="text-sm text-slate-500 mt-2">
                    No documents uploaded yet. Please upload documents in the Document Upload section first.
                  </p>
                )}
              </div>
            </div>
      ...
      {/* Analysis Results */}
      {(analysisStatus !== 'idle' || selectedFileId) && (
              ...
              {extractedRequirements.length > 0 ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-600 mb-4">
                    {selectedFileId 
                      ? "No requirements have been generated for this document yet. Click 'Extract Requirements' to analyze the document."
                      : "Please select a document first to view or generate requirements."
                    }
                  </p>
                  {selectedFileId && (
                    <button 
