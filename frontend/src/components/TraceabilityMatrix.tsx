...
const TraceabilityMatrix: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setError('');
    apiService.getTestCasesByFile(selectedFileId)
      .then((data: any) => {
        setMatrixData(data);
    ...
    <div className="p-4 max-w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-4 mb-6">
        <div className="flex flex-col w-full md:w-1/3">
          <label htmlFor="file-select" className="font-semibold mb-1">Select a Document</label>
          <select
            id="file-select"
            className="border rounded px-3 py-2 focus:outline-none focus:ring"
            value={selectedFileId}
            onChange={e => { setSelectedFileId(e.target.value); setHighlightedReqId(''); setExpandedTcId(''); }}
            disabled={loadingFiles}
            aria-label="Select a Document"
          >
            <option value="">{loadingFiles ? 'Loading...' : 'Choose a document'}</option>
            {files.map(f => (
